---
title: Episode 107 – PowerWF v2 with Ben and Brian
author: Jonathan Walz
authors:
  - Jonathan Walz
date: "2010-03-30T16:14:17+00:00"
podcast_url: "http://traffic.libsyn.com/powerscripting/PSPodcast-107.mp3"
aliases:
  - /2010/03/episode-107-powerwf-v2-with-ben-and-brian/
---

**A Podcast about Windows PowerShell.**  
Listen:


  [**![](http://powerscripting.libsyn.com/img/podcastIcon.gif)**](http://traffic.libsyn.com/powerscripting/PSPodcast-107.mp3)



 








## 

In This Episode







Tonight on the PowerScripting Podcast we talk to Ben and Brian from PowerWF about PowerWF v2!





## 

News






     





    *

Before you acquire power, you must acquire knowledge "“ and Quest Software has what you need! In Jeffery Hicks"™ e-book, "Managing Active Directory with Windows PowerShell," learn how PowerShell helps you master local accounts and groups, password management, security and permissions and much, much more. You"™ll also learn about the PowerShell extras and out-of-the-box features that will help you control Active Directory.

*





    *

Don"™t wait "“ visit 
[
quest.com/powershellbook
](http://www.quest.com/powershellbook)
 to register for your free copy and see why PowerShell and Quest Power GUI are the ultimate Windows management tools.

*





    *
 *







      - 
        [Quest PowerGUI Pro is now shipping](http://www.quest.com/PowerGUIPro/)


      - 
        Thomas Lee has added another
 
[PowerShell Master Class session in Oslo June 22-24](http://tfl09.blogspot.com/2010/03/powershell-master-class-more-sessions.html)


      - 
        [Scripting Games 2010!](http://blogs.technet.com/heyscriptingguy/archive/2010/03/20/hey-scripting-guy-march-20-2010.aspx)


      - 
        [Lee Holmes' PowerShell Cookbook v2 is in open beta!](http://powershell.labs.oreilly.com/)


      - 
        Ed Wilson will be presenting at the
 
[PowerShell Usergroup Brisbane](http://psugbne.org/)








## 
    Interview





    *

Version 2.0 of PowerWF Studio has just been released. PowerWF is a Visual PowerShell development tool allowing users to build PowerShell scripts as workflows, which are similar to a flowchart or Visio diagram.  Once created, workflows can be deployed as applications, run as a scheduled task, or back to PowerShell as a script.  You can even push data to System Center with the click of a button. To get your free 30 day trial go to [PowerWF.com](http://powerwf.com/).

*




#### 
    Links





    - 
      PowerWF Home: http://powerwf.com/


    - 
      Product matrix: [http://powerwf.com/product_list/product-comparison/](http://powerwf.com/product_list/product-comparison/)






#### 

Questions






    - 
      meson: ## Any new products in the pipeline for PowerWF?


    - 
      meson: ## Does it support PowerShell Remoting?






## 

Resources






    *This segment is brought to you by *[
*SAPIEN Technologies*
](http://sapien.com/)*.*






*
 *






    - 
      New blog from one of the SQLISE developers:
 
[PauerSchell](http://pauerschell.blogspot.com/)


    - 
      [The SharePoint PodShow Episode 44](http://www.sharepointpodshow.com/archive/2010/03/18/sharepoint-2010-and-powershell-episode-44.aspx)
 
talks about PowerShell support in v2010.


    - 
      SharePoint MVP Gary Lapointe has a cool
 
[SharePoint automation blog](http://stsadm.blogspot.com/)
 
you should follow


    - 
      [PowerShell + iPhone + Prowl = SysAdmin notification nirvana](http://janssenjones.typepad.com/janssenjonescom/2009/10/powershell-iphone-prowl-sysadmin-notification-nirvana.html)


    - 
      You can
 
[download the help file for the new Exchange 2010 cmdlets](http://powershell.com/cs/blogs/tips/archive/2010/03/24/exchange-2010-compiled-help.aspx)







    **


**





    **
Tips
**















Our tips are brought to you today by ServerFault and StackOverflow, the best place to find answers to your toughest questions!















    - 
      [Command Line Kung Fu - Episode 87: Making a Hash of Things](http://blog.commandlinekungfu.com/2010/03/episode-87-making-hash-of-things.html)


    - 
      Doug Finke
 
[shows us how to use WPK to create a GUI](http://blogs.technet.com/heyscriptingguy/archive/2010/03/24/hey-scripting-guy-march-24-2010.aspx)







    
