---
title: Episode 152 – PrimalForms 2011 with Alex and David
author: Jonathan Walz
authors:
  - Jonathan Walz
date: "2011-07-08T03:18:54+00:00"
podcast_url: "http://traffic.libsyn.com/powerscripting/PSPodcast-152.mp3"
aliases:
  - /2011/07/episode-152-primalforms-2011-with-alex-and-david/
---

**A Podcast about Windows PowerShell.**
 Listen:


  [**![](http://powerscripting.libsyn.com/img/podcastIcon.gif)**](http://traffic.libsyn.com/powerscripting/PSPodcast-152.mp3)



## 
    In This Episode





    Tonight on the PowerScripting Podcast, we talk to Alex and David from Sapien about PrimalForms 2011




## 
    News





    This segment brought to you by Start-Automating
 Start Scripting to Your Fullest Potential.  At Start-Automating, we can help you unleash the full Power of PowerShell V2.  You can use our deep PowerShell expertise to build rich PowerShell solutions, or we can train you to use PowerShell like a pro. Isn"™t it time you Save-Time, Save-Money, and Start-Automating?  Find out more at [Start-Automating.com](http://start-automating.com/).





    - 
      Did you know there"™s a [PowerShell Facebook group](https://www.facebook.com/home.php?sk=group_119812241410296)?


    - 
      Don Jones"™ latest book: [Learn PowerShell in a Month of Lunches](http://www.manning.com/jones/) is now available in epub and mobi formats






## 
    Interview





    This segment is brought to you by PowerGUI Pro with MobileShell, Version Control, and Easy Remote Script Execution.
 At Quest we are passionate about Windows PowerShell. PowerGUI Pro enables organizations to harness the power of PowerShell without the expense of training and custom scripts and applications. PowerGUI Pro solves issues regardless of the time and place by using MobileShell to remotely manage your infrastructure. Ensure scripting best practices by leveraging integration with popular Version Control systems. Automate against thousands of computers using Easy Remote Script Execution. [Get PowerGUI Pro at quest.com/powerguipro](http://www.quest.com/PowerGUIPro/)
 Links:





    - 
      [PrimalForms](http://www.sapien.com/software/primalforms)


    - 
      [SAPIEN blog](http://www.sapien.com/blog)






##### 
    Buzz from the Chatroom:





    [21:47]  ## PrimalForms is awesome sauce!
 [21:48] <@powerscripting> He's SAPIENDavid on twitter
 [21:50]  ##Who is the target user for SAPIEN products?
 [21:51]  ## Are there plans for a WPF version of PrimalForms (I had to ask.)
 [21:54]  ## Will PrimalForms support WPF in the future?
 [21:55]  ## When will Primalforms support WPF?
 [21:56]  ## Alex do you still do development?
 [21:57]  I like to see a CTO that is actually technical and still active in the technology
 [21:59]  WPF is .Net 3.0+
 [21:59] <@scriptingwife> @Bartek are you awake?
 [22:02]  ## Any thoughts on the upcoming Build Conference. Rumors on WPF being "upgraded" to DirectUI etc?
 [22:04]  ##  I'm admin and I want to use WPF. It looks like Win7 element, so users don't feel like in time machine...
 [22:05]  ## Speaking of Windows 8 -- do you think PowerShell + WPF or Winforms can develop apps for the new Direct UI?
 [22:06]  #### Primal for iPad?  I need to write scripts on the go! 🙂
 [22:08]  ## What do you see as the main use of PrimalForms (or, more generally, of administrators building UI in PowerShell)?  Is it asking for input? Showing output (charts, graphs)? Interactive apps (twitter, notepad)?
 [22:08]  ## Any upcoming changes for the #iOS app?
 [22:11]  ##Does PrimalForms integrate with source control?
 [22:17]  ##Will that be ported to Android???
 [22:25]  I have some PrimalForm posts on my blog [http://jdhitsolutions.com/blog/?s=primalforms](http://jdhitsolutions.com/blog/?s=primalforms)




##### 
    Questions:





    Computer Alex: Tandy TRS-80
 Superhero David: Spiderman, no Hiro! No, Wolverine!




### 
    Resources





    - 
      Boe Prox shows us how to translate [Windows error codes with PowerShell](http://learn-powershell.net/2011/06/28/quick-hits-translate-windows-error-codes/)


    - 
      [Start-Automating highlights a Get-Protocol script ](http://blog.start-automating.com/post/2011/06/28/Get-Protocol.aspx)and lots of tips about how the script was made


    - 
      Doug Finke shows us a [cool ShowUI Twitter example](http://www.dougfinke.com/blog/index.php/2011/06/26/powershell-showui-and-the-twitter-api/)


    - 
      The ScriptingGuy"™s [favorite PowerShell tricks](http://blogs.technet.com/b/heyscriptingguy/archive/2011/06/18/top-ten-favorite-powershell-tricks-part-1.aspx)






### 
    Tips





    - 
      Jeff Hicks shows us how to [use ScriptBlocks on the fly](http://jdhitsolutions.com/blog/2011/06/scriptblocks-on-the-fly/)







    - 
      [The Magic of PowerShell Casting](http://csharpening.net/?p=524)


    - 
      What to do when the[ ISE hangs on start up](http://justtinkering.wordpress.com/2011/06/16/powershell_ise-hangs-on-start-up/)


    - 
      "[Tied Variables](http://robertrobelo.wordpress.com/2011/06/24/tied-variables-a-simpler-way/)"







    Gotcha





    - 
      Get-WmiObject -ErrorAction "SilentlyContinue"


    - 
      Target location
