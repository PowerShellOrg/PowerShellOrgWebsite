---
title: Episode 193 – Hemant Mahawar from Microsoft on PowerShell v3 Workflows
author: Jonathan Walz
authors:
  - Jonathan Walz
date: "2012-07-18T03:52:28+00:00"
podcast_url: "http://traffic.libsyn.com/powerscripting/PSPodcast-193.mp3"
aliases:
  - /2012/07/episode-193-hemant-mahawar-from-microsoft-on-powershell-v3-workflows/
---

**A Podcast about Windows PowerShell.**
 Listen:


  **[![](http://powerscripting.libsyn.com/img/podcastIcon.gif)](http://traffic.libsyn.com/powerscripting/PSPodcast-193.mp3)**



## 
    In This Episode





    **Tonight on the PowerScripting Podcast, we talk to Hemant Mahawar from Microsoft about PowerShell v3 Workflows
 **




## 
    News





    This segment is brought to you by TrainSignal





    Get online computer based training for you and your whole staff from TrainSignal. TrainSignal offers more than 80 IT training courses in the areas of Security, Network Administration, Virtualization, PowerShell and more. With 24/7 online access, TrainSignal"™s training courses are accessible on almost any device including PC, Mac, iPhone, iPad, Android smartphones, and more.  For more info and a list of courses, visit[ TrainSignal.com](http://trainsignal.com/).





    **
 **





    - 


        The Dallas/Fort Worth PowerShell group is [meeting July 21st](http://powershellgroup.org/content/dallasfort-worth-meeting-july-21st-2012-8-10am)





    - 


        Bangalore has a PowerShell user group! [First meeting July 28th](http://www.ravichaganti.com/blog/?p=2584&utm_source=feedburner&utm_medium=feed&utm_campaign=Feed%3A+RavikanthChaganti+%28From+Ravikanth%27s+Blog%29).





    - 


        The UK PowerShell Group is meeting [virtually on July 31st](http://msmvps.com/blogs/richardsiddaway/archive/2012/07/11/uk-powershell-group-session-31-july-2012.aspx).





    - 


        Limited time ["PowerShell in Depth" offer](http://store.concentratedtech.com/indepth.php)





    - 


        Doug Finke"™s "[Windows PowerShell for Developers](http://shop.oreilly.com/product/0636920024491.do)" has been released!








## 
    Interview





    This segment brought to you by Start-Automating





    **
 **





    Start Scripting to Your Fullest Potential.  At Start-Automating, we can help you unleash the full Power of PowerShell V2.  You can use our deep PowerShell expertise to build rich PowerShell solutions, or we can train you to use PowerShell like a pro. Isn"™t it time you Save-Time, Save-Money, and Start-Automating?  Find out more at [Start-Automating.com](http://start-automating.com/).





    **
 **





    Guests - Hemant Mahawar, Microsoft




#### 
    Links





    - 


        [http://repository.tamu.edu/bitstream/handle/1969.1/5930/etd-tamu-2006A-CPSC-Mahawar.pdf](http://repository.tamu.edu/bitstream/handle/1969.1/5930/etd-tamu-2006A-CPSC-Mahawar.pdf)





    - 


        [http://channel9.msdn.com/Events/TechEd/NorthAmerica/2012/WSV414](http://channel9.msdn.com/Events/TechEd/NorthAmerica/2012/WSV414)








#### 
    The Question -





    Superhero: He-Man





    **Chatroom Banter**





    **
 **





    ScriptingWife: You all will be so proud. We went ot see Spiderman. AND I loved it.





    JonWalz: you said "to release is to ship"





    halr9000: aieee





    halr9000: doh!





    JonWalz: come over to the developer side, we have cookies!





    Jaykul: ## Is the workflow endpoint created when you install powershell 3, or when you enable-psremoting?





    JuneB: Can I run any command in a workflow?





    Vern_Anderson: ## How does it know how to continue or resume even after reboot





    Jaykul: ## Have they published examples of using a 3rd party workflow activity?





    Vern_Anderson: ## how are variables remembered after a reboot





    JuneB: Activity is the unit of work in a workflow





    Mike_F_Robbins: "Going forward saying we're not going to use remoting is like saying we're not going to use Ethernet" - Don Jones (TechEd 2012)





    JuneB: ## Tell us about Invoke-AsWorkflow





    Daniel: ## Are the built-in workflow activities intended be used in PowerShell?




#### 
    Resources





    - 


        Microsoft has released some Quick Reference guides for [PowerShell 3.0](http://www.powershellmagazine.com/2012/06/09/windows-powershell-3-0-and-server-manager-quick-reference-guides/) and [Server Manager](http://craigwilson.id.au/2012/07/04/windows-powershell-3-0-and-server-manager-quick-reference-guides/)





    - 


        MVP Max Trinidad has posted some [resource links for PowerShell 3.0](http://www.maxtblog.com/2012/07/windows-powershell-v3-0-resource-links-start-now/)





    - 


        Don Jones talks about the [top 3 features in PowerShell 3.0](http://cloudsns.wordpress.com/2012/06/13/don-jones-top-3-features-in-powershell-v3/)
